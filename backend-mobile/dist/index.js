...
import { askCoach, getPepTalk, getGoalAdvice as aiGoalAdvice, generateWeeklyReview as aiWeeklyReview, } from './ai.js';
...
try {
    db.exec('ALTER TABLE goals ADD COLUMN emoji TEXT DEFAULT \'🎯\'');
}
catch { }
// Seed test user
...
function buildContext(userId) {
    ...
    const morningCI = db.prepare("SELECT * FROM check_ins WHERE user_id = ? AND date = ? AND type = 'morning'").get(userId, t);
    const eveningCI = db.prepare("SELECT * FROM check_ins WHERE user_id = ? AND date = ? AND type = 'evening'").get(userId, t);
    return {
...
app.get('/api/habits', (req, res) => {
    try {
        const userId = getUser(req);
        const t = today();
        const habits = db.prepare('SELECT * FROM habits WHERE user_id = ? ORDER BY created_at').all(userId);
...
app.post('/api/habits/:id/complete', (req, res) => {
    try {
        ...
        const { id } = req.params;
        const t = today();
        const habit = db.prepare('SELECT * FROM habits WHERE id = ? AND user_id = ?').get(id, userId);
        if (!habit)
        ...
        if (habit.last_completed === t) {
            ...
            const all = db.prepare('SELECT last_completed FROM habits WHERE user_id = ?').all(userId);
...
app.get('/api/checkins', (req, res) => {
    try {
        ...
        const checkins = db.prepare('SELECT * FROM check_ins WHERE user_id = ? AND date = ?').all(userId, date);
        res.json({ checkins });
    }
    catch (e) {
        res.status(400).json({ error: String(e) });
...
app.post('/api/chat/message', async (req, res) => {
    try {
        const { text } = req.body;
        const userId = getUser(req);
        const user = db.prepare('SELECT personality FROM users WHERE id = ?').get(userId);
        const pers = (user?.personality || 'stoic');
...
app.post('/api/chat/pep-talk', async (req, res) => {
    try {
        const userId = getUser(req);
        const user = db.prepare('SELECT personality FROM users WHERE id = ?').get(userId);
        const pers = (user?.personality || 'stoic');
...
app.post('/api/chat/goal-advice', async (req, res) => {
    try {
        ...
        const goal = goalId
            ? db.prepare('SELECT * FROM goals WHERE id = ? AND user_id = ?').get(goalId, userId)
            : null;
        const advice = await aiGoalAdvice(pers, ctx, goal?.name || 'your goal', goal?.progress || 0, goal ? Math.floor((Date.now() - new Date(goal.created_at).getTime()) / 86400000) : 0);
        res.json({ advice });
    }
    catch (e) {
        res.status(400).json({ error: String(e) });
...
app.get('/api/stats/week', (req, res) => {
    try {
        const userId = getUser(req);
        const user = db.prepare('SELECT dopamine_score FROM users WHERE id = ?').get(userId);
        const score = user?.dopamine_score ?? 50;
        // Build week data from habit completions
        const habits = db.prepare('SELECT last_completed FROM habits WHERE user_id = ?').all(userId);
        const weekData = [];
...
app.get('/api/stats/leaderboard', (req, res) => {
    try {
        const userId = getUser(req);
        const user = db.prepare('SELECT name, dopamine_score FROM users WHERE id = ?').get(userId);
...
app.post('/api/review/generate', async (req, res) => {
    try {
        ...
        const habits = db.prepare('SELECT * FROM habits WHERE user_id = ?').all(userId);
        const goals = db.prepare('SELECT * FROM goals WHERE user_id = ?').all(userId);
        const doneCount = habits.filter((h) => h.last_completed === t).length;
        ...
        const failedCIs = db.prepare("SELECT reason FROM check_ins WHERE user_id = ? AND type = 'evening' AND completed = 0 AND date >= ?").all(userId, weekAgoStr);
        const failedReasons = failedCIs.map((c) => c.reason).filter(Boolean);
        // Stalled goals
        ...
        const topStreak = habits.reduce((max, h) => Math.max(max, h.streak_days || 0), 0);
        const topHabit = habits.find((h) => h.streak_days === topStreak);
        const insights = [
